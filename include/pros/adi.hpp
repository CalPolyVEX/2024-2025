...
class Potentiometer : public AnalogIn {
  ...
  public:
     ...
     * \endcode
     */
    explicit Potentiometer(ext_adi_port_pair_t port_pair,
...
class Pneumatics : public DigitalOut {
  ...
  public:
     ...
     * \code
     * void opcontrol() {
     * 	 pros::adi::Pneumatics left_piston('a', false);			// Starts retracted, extends when the ADI port is
     *high pros::adi::Pneumatics right_piston('b', false, true);	// Starts retracted, extends when the ADI port
     *is low
     *
     ...
     * \code
     * void opcontrol() {
     * 	 pros::adi::Pneumatics left_piston({1, 'a'}, false);			// Starts retracted, extends when the ADI
     *port is high pros::adi::Pneumatics right_piston({1, 'b'}, false, true);	    // Starts retracted, extends
     *when the ADI port is low
     *
     *   pros::Controller master(pros::E_CONTROLLER_MASTER);
     *
     *   while (true) {
     ...
     * \code
     * void opcontrol() {
     * 	 pros::adi::Pneumatics piston({1, 'a'}, false);            // Starts retracted, extends when the ADI port is
     * high
     *
     *   pros::Controller master(pros::E_CONTROLLER_MASTER);
     *
