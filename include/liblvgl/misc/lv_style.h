...
enum {
  ...
  LV_DITHER_ERR_DIFF, /**< Error diffusion mode. Slower to compute and use more memory but give highest dither
                         quality*/
};
 ...
 * @param user_data any custom data that will be saved in the transition animation and will be available when `path_cb`
 * is called
 * @example
...
static inline lv_style_res_t lv_style_get_prop_inlined(const lv_style_t* style, lv_style_prop_t prop,
  ...
  if (style->prop1 == LV_STYLE_PROP_ANY) {
    ...
    for (i = 0; i < style->prop_cnt; i++) {
      ...
      if (prop_id == prop) {
        if (const_prop->prop & LV_STYLE_PROP_META_INHERIT) return LV_STYLE_RES_INHERIT;
        *value =
            (const_prop->prop & LV_STYLE_PROP_META_INITIAL) ? lv_style_prop_get_default(prop_id) : const_prop->value;
  ...
  if (style->prop_cnt > 1) {
    ...
    for (i = 0; i < style->prop_cnt; i++) {
      ...
      if (prop_id == prop) {
        if (props[i] & LV_STYLE_PROP_META_INHERIT) return LV_STYLE_RES_INHERIT;
        if (props[i] & LV_STYLE_PROP_META_INITIAL) *value = lv_style_prop_get_default(prop_id);
        else {
    ...
    if (style->prop1 & LV_STYLE_PROP_META_INHERIT) return LV_STYLE_RES_INHERIT;
...
/**
 * Tell the group of a property. If the a property from a group is set in a style the (1 << group) bit of
 * style->has_group is set. It allows early skipping the style if the property is not exists in the style at all.
