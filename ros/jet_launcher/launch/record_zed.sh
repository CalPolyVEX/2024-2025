#!/bin/bash
if [ "$#" -ne 1 ]; then
   rosbag record --duration=2m /zed/data_throttled_image_depth /zed/data_throttled_image /zed/data_throttled_camera_info /zed/zed_node/depth/camera_info /camera/odom/sample /tf /tf_static 
