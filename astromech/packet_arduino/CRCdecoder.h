...
unsigned short int max_byte_count = 0;
...
void get_input()
{
  // If USB is recieving data, collect data
	if (SerialUSB.available())
	{
		...
		while (SerialUSB.available() && (!byte_count || byte_count != max_byte_count))
		{
        ...
        continue;
			...
			// If current byte is the num bytes, allocate respective memory
			if (byte_count)
			{
