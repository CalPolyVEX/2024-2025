...
import augment_goal_head
import pretrained_model_goal_head
...
if __name__ == '__main__':
    ...
    train_fp, val_fp = augment_goal_head.setup_dir()
    train_d, val_d = augment_goal_head.create_datasets(train_fp, val_fp)
    ...
    dataloaders = {x: torch.utils.data.DataLoader(image_datasets[x], \
                   ...
                   batch_size=1, shuffle=True, num_workers=1) for x in ['train', 'val']}

    dataset_sizes = {x: len(image_datasets[x]) for x in ['train', 'val']}
    print (dataset_sizes)

    #create the model
    m = pretrained_model_goal_head.Goal_Model(shape=(360,640,3), num_outputs=82)
    model = m.build()
