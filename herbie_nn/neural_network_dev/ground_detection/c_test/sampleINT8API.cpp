...
void writeNetworkTensorNames(nvinfer1::INetworkDefinition *network)
{
    ...
    std::ofstream tensorsFile{"network_tensors.txt"};
...
bool setDynamicRange(nvinfer1::INetworkDefinition *network)
{
    ...
    {
        std::string tName = network->getInput(i)->getName();
        std::cout << "input tensor: " << tName << std::endl;
        if (!network->getInput(i)->setDynamicRange(0,1))
        {
           return false;
        }
    ...
    for (int i = 0; i < network->getNbLayers(); ++i)
    {
        ...
        {
            std::string tName = lyr->getOutput(j)->getName();
            std::cout << "output tensor: " << tName << std::endl;
                if (!lyr->getOutput(j)->setDynamicRange(0,2))
                {
                    return false;
                }
        }
