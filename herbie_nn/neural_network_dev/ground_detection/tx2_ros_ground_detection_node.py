...
class image_inference:
   ...
   def callback(self,msg_in):
      ...
      end_time = time.time()
      print ('FPS: ' + str(1.0/(end_time - start_time)))

      self.point_list = []
      for c in range(0,80):
          cv2.circle(image_np, ((c*8), round(float(output.data[0][c]) * (360-1))), 1, (0, 255, 0), -1)
      ...
      self.connect_boundary(self.point_list,image_np)
      self.local_planner(image_np)

      cv2.putText(image_np,str(f'{ms_time:.2f}') + 'ms FPS: ' + str(fps),
          (430,25),
          font,
          fontScale,
          fontColor,
