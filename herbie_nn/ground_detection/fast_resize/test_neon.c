...
int main() {
   ...
   for (int j=0;j<1000;j++) {
      ...
      for(int row=0; row<360; row++) 
      {
         ...
         for(int column=0; column<40; column++) //40 iterations per row (1920*2 / 96)
         {
            ...
            for (int i=0; i<4; i++) {
               //store blue values
               unsigned int val = vgetq_lane_u32(b_low_int, i);
            ...
            for (int i=0; i<4; i++) {
               //store blue values
               unsigned int val = vgetq_lane_u32(b_high_int, i);
