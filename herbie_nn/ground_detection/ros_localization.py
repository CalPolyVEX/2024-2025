...
class image_inference:
   def __init__(self, model_name = "test.pt", unlabeled = False):
   ...
   def callback(self,msg_in):
      ...
      self.max_val = 0
      for c in range(64):
         val = float(output.data[0][c])
         if val > self.max_val:
            self.max_val = val
            self.max_index = c
      self.straight_percentage = int(100 * float(output.data[0][64]))
      self.turn_percentage = int(100 * float(output.data[0][65]))
      ...
      cv2.putText(image_np_360,str(self.max_index) + '  ' + "{:.3f}".format((self.max_val)),
          (430,50),
          font,
          .8,
          (255,0,255),
