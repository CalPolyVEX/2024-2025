...
class CameraReader {
  ...
  int init(int argc, char **argv) {
   ...
   bgr_frame_360 = Mat(height/3, width/3, CV_32FC3);

   return 0;
  }

  void frame_loop(ObstacleDetection* od) {
    while(ros::ok()) {
      usleep(45000);
      //usleep(40000);
