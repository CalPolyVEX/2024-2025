...
def init_program():
   ...
   #get image data
   image_list = os.listdir("640_images")
   image_list.sort()
   for f in image_list:
      if f.endswith(".jpg"):
         filename = os.path.join("640_images/", f)
...
#test convert to grayscale
   ...
   for i in range(len(image_list)):
      ...
      image = image.replace("640_","")
      datafile = datafile.replace(".txt","")
      datafile = datafile.replace("640_gt_","")
   ...
   for f in data_list:
      if f.endswith(".txt"):
         filename = os.path.join("640_data/", f)
