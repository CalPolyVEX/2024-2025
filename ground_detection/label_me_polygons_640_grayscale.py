...
if sys.argv[1] == 'clean':
    print "clean"
    os.system('cd ' + sys.argv[2] + '/640_input; rm *.jpg')
    os.system('cd ' + sys.argv[2] + '/640_data; rm *.txt')
    os.system('cd ' + sys.argv[2] + '/640_ground_truth; rm *.jpg')
    os.system('cd ' + sys.argv[2] + '/640_inference_input; rm *.jpg')
    os.system('cd ' + sys.argv[2] + '/640_final_inference_output; rm *.jpg')
...
def build_640_480_images(in_dir):
    print "Converting input images to 640x480"
    files = os.listdir(in_dir)
    files.sort()

    #create directory for 640x480 input files
    input_dir_640 = sys.argv[1]+"/640_input"
    if not os.path.exists(input_dir_640):
        os.makedirs(input_dir_640)

    for f in files:  #resize all the input files
        img = cv2.imread(in_dir + '/' + f)
        img640 = cv2.resize(img,(640,480), interpolation=cv2.INTER_CUBIC)
        new_name = '640_' + f
        cv2.imwrite(input_dir_640 + '/' + new_name, img640)

def build_640_480_gt_images(gt_dir):
    print "Converting ground truth images to 640x480"
    gt_files = os.listdir(gt_dir)
    gt_files.sort()

    #create directory for 640x480 ground truth files
    gt_dir_640 = sys.argv[1]+"/640_ground_truth"
    if not os.path.exists(gt_dir_640):
        os.makedirs(gt_dir_640)

    for f in gt_files:  #resize all the ground_truth files
        img = cv2.imread(gt_dir + '/' + f)
        img640 = cv2.resize(img,(640,480), interpolation=cv2.INTER_CUBIC)
        new_name = '640_' + f 
        cv2.imwrite(gt_dir_640 + '/' + new_name, img640)
